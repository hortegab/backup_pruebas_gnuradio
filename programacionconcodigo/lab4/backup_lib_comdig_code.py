...
class vec_diagrama_ojo2_f(gr.sync_block):  
    ...
    """hecho por: Homero Ortega Boada. Permite obtener el diagrama de ojo.
    Esta version se diferencia de vec_diagrama_ojo_f en que aqui no se usa
    la libreria eyediagram sino que todo se hace a punta de matplotlib.
...
class diagrama_ojo2_e_f(gr.sync_block):  
    # NOTA: Este bloque lo hicimos pensando que hacer una version tipo stream
    # a partir de la version vectorial seria muy sencilla, simplemente
    # * considerar 2*sps muestras a la hora de graficar
    # * en la libreria comdiguis tenemos eso en b_eye_diagram2_f
    # NOTA: Este bloque no sirvio. La hipotesis es que por no ser vectorial
    # no es seguro que la senal entrante arranque en un instante predecible
    # por ejemplo, uno puede ver que un bit arranca al inicio, pero con
    # el tiempo estara arrando a mitad del bit o en otra parte
    # lo raro es que con GRC no pasa eso.
    """hecho por: Homero Ortega Boada. Permite obtener el diagrama de ojo"""

    def __init__(self, Sps=8, Delay=0, samp_rate=32000):
        gr.sync_block.__init__(self, name='diagrama_ojo2_e_f', in_sig=[np.float32], out_sig=None)
        self.Sps = Sps
        self.Delay=Delay
        self.Tsamp=1./samp_rate
        self.L=2*Sps
        Tb=self.Sps*self.Tsamp
        self.t = np.linspace(-Tb,Tb, self.L)
        

    def work(self, input_items, output_items):
        in0 = input_items[0] # in0 es un 1D array
        s=in0[self.Delay:self.Delay+self.L] # es la pequna porcion a graficar
        plt.plot(self.t,s)             
